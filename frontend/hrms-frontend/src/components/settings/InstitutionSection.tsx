...
export default function InstitutionSection() {
  const { data, loading, refetch } = useQuery<{ settings: InstitutionSettings }>(GET_SETTINGS);
  ...
  const settings = data?.settings || {} as InstitutionSettings;
  ...
  const startEditing = () => {
    setFormData({
      ...
      address: {
        ...
        pin_code: settings.address?.pin_code || "",
      },
  ...
  const handleDayToggle = (day: string) => {
    ...
    if (current.includes(day)) {
      setFormData(prev => ({ ...prev, working_days: current.filter(d => d !== day) }));
    } else {
      setFormData(prev => ({ ...prev, working_days: [...current, day] }));
    }
  };

  const renderField = (label: string, name: keyof InstitutionSettings, value: string) => {
    const isReadOnly = ["added_by", "created_at", "id"].includes(name);
    return (
      <div className="info-item">
        <span className="info-label">{label}</span>
        {isEditing && !isReadOnly ? (
          <input name={name} value={(formData[name] as string) || ""} onChange={handleChange} className="inst-input" />
        ) : (
    ...
    <div className="inst-container">
      ...
      <div className="inst-header">
        ...
        {isEditing ? (
          ...
          <button className="inst-edit-btn" onClick={startEditing}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div className="inst-card" style={{ flex: 1 }}>
            <div className="inst-logo-wrapper">
              <div className="inst-logo-circle">
                {(isEditing ? formData.institution_logo : settings.institution_logo) ? (
                  <img src={isEditing ? formData.institution_logo : settings.institution_logo} alt="Logo" />
                ) : (
              ...
              <div className="inst-logo-text">
                {isEditing ? (
                  ...
                  settings.institution_name || "New Institution"
                )}
              </div>
          ...
          <div className="inst-card">
            ...
            <div className="info-grid" style={{ gridTemplateColumns: '1fr' }}>
              {renderField("PAN", "pan_number", (isEditing ? formData.pan_number : settings.pan_number) || "")}
              {renderField("Registration No", "registration_number", (isEditing ? formData.registration_number : settings.registration_number) || "")}
              {renderField("College Code", "institution_code", (isEditing ? formData.institution_code : settings.institution_code) || "")}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div className="inst-card">
            ...
            <div className="info-grid">
              {renderField("Organization Name", "institution_name", (isEditing ? formData.institution_name : settings.institution_name) || "")}
              <div className="info-item">
                <span className="info-label">Institution ID</span>
                <span className="info-value">{settings.institution_id || "—"}</span>
              </div>
              {renderField("Owner Name", "owner_name", (isEditing ? formData.owner_name : settings.owner_name) || "")}
              <div className="info-item">
                <span className="info-label">Added By</span>
                <span className="info-value">{settings.added_by || "System Admin"}</span>
              </div>
              {renderField("Short Name", "institution_short_name", (isEditing ? formData.institution_short_name : settings.institution_short_name) || "")}
              <div className="info-item">
                <span className="info-label">Added Time</span>
                <span className="info-value">{settings.created_at ? new Date(parseInt(settings.created_at)).toLocaleString() : "—"}</span>
              </div>
            </div>
          </div>
          ...
          <div className="inst-card">
            ...
            <div className="info-grid">
              {renderField("Organization email", "contact_email", (isEditing ? formData.contact_email : settings.contact_email) || "")}
              {renderField("Organization mobile", "contact_mobile", (isEditing ? formData.contact_mobile : settings.contact_mobile) || "")}
              {renderField("Organization phone", "contact_phone", (isEditing ? formData.contact_phone : settings.contact_phone) || "")}
              <div className="info-item" style={{ gridColumn: '1 / -1' }}>
                ...
                {!isEditing ? (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '4px' }}>
                    <input name="address.line1" value={formData.address?.line1 || ""} onChange={handleChange} className="inst-input" placeholder="Line 1" />
                    <input name="address.line2" value={formData.address?.line2 || ""} onChange={handleChange} className="inst-input" placeholder="Line 2" />
                    <input name="address.city" value={formData.address?.city || ""} onChange={handleChange} className="inst-input" placeholder="City" />
                    <input name="address.state" value={formData.address?.state || ""} onChange={handleChange} className="inst-input" placeholder="State" />
                    <input name="address.country" value={formData.address?.country || ""} onChange={handleChange} className="inst-input" placeholder="Country" />
                    <input name="address.pin_code" value={formData.address?.pin_code || ""} onChange={handleChange} className="inst-input" placeholder="Pin Code" />
                  </div>
                )}
              </div>
              {renderField("Website", "website_url", (isEditing ? formData.website_url : settings.website_url) || "")}
              {renderField("Fax Number", "fax_number", (isEditing ? formData.fax_number : settings.fax_number) || "")}
            </div>
          </div>
          ...
          <div className="inst-card">
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {DAYS.map(day => {
                const currentDays = isEditing ? (formData.working_days || []) : (settings.working_days || []);
