...
import RelievingStatsSection from "./components/sections/RelievingStatsSection";
import RelievingFilterSection from "./components/sections/RelievingFilterSection";
import RelievingTableSection from "./components/sections/RelievingTableSection";
import RelievingPaginationSection from "./components/sections/RelievingPaginationSection";
...
const CSS = `
  ...
  /* ── Status Badge ── */
  .er-badge { padding:3px 8px; border-radius:6px; font-family:'DM Sans',sans-serif; font-size:11px; font-weight:700; }
  .er-badge-pending   { background:#fef3c7; color:#b45309; }
  .er-badge-approved  { background:#dcfce7; color:#15803d; }
  .er-badge-clearance { background:#eff6ff; color:#1d4ed8; }
  .er-badge-relieved  { background:#f1f5f9; color:#475569; }
  .er-badge-rejected  { background:#fef2f2; color:#dc2626; }
  ...
  return (
    <div className="er-page">
      ...
      <RelievingHeaderSection
        isAdmin={isAdmin()}
        onExport={() => showToast("Exit report exported.")}
        onRecordExit={() => setFormOpen((v: boolean) => !v)}
      />
      ...
      {/* Stat Cards */}
      <RelievingStatsSection stats={stats} />

      {/* Table Section */}
      <div className="er-section">
        {/* Filter Bar */}
        <RelievingFilterSection
          filters={filters}
        ...
        <RelievingTableSection
          ...
          actions={{
            ...
            onReject: (rec) =>
              initiateReject(rec.id, `${rec.firstName} ${rec.lastName}`, ""),
            onRelieve: (rec) =>
              initiateRelieve(rec.id, `${rec.firstName} ${rec.lastName}`),
          }}
        />

        {/* Pagination */}
        <RelievingPaginationSection
