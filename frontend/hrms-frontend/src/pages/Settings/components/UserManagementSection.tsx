...
export default function UserManagementSection() {
  ...
  const groupedUsers = useMemo(() => {
    ...
    let deptUsersList = users.filter(
      (u) => u.role === "Hod" || u.role === "Member",
    );
    const term = search.toLowerCase();
    if (term) {
      deptUsersList = deptUsersList.filter(
        (u) =>
          ...
          <tbody>
            {userList.map((u) => (
              <tr key={u.id}>
                ...
                <td>
                  <span
                    className={`um-status-badge ${u.status === "Active" ? "um-status-active" : "um-status-inactive"}`}
                  >
                    {u.status}
                  </span>
                </td>
    ...
    <div className="um-section">
      ...
      <div className="um-header">
        ...
        <div className="um-tabs">
          ...
          <div
            className={`um-tab ${activeTab === "Admin" ? "active" : ""}`}
      ...
      {activeTab === "Users" && (
        <>
          {Object.keys(groupedUsers).length > 0 ? (
            Object.entries(groupedUsers).map(([dept, deptUsers]) => (
              <div key={dept} className="um-dept-group">
                <div className="um-dept-title">
                  {dept}{" "}
                  <span className="um-dept-badge">{deptUsers.length}</span>
                </div>
            ...
            >
              No assigned users found matching your search. Use "Assign User" to
              add them.
            </div>
          )}
        </>
          ...
          {filteredAdmins.length > 0 ? (
            <div className="um-dept-group">
              <div className="um-dept-title">
                ...
                <span className="um-dept-badge">{filteredAdmins.length}</span>
              </div>
          ...
          <div className="um-modal">
            ...
            <div className="um-modal-body">
                ...
                >
                  ...
                  {users.map((u) => (
                    <option key={u.id} value={u.id}>
                      {u.name} ({u.email}) -{" "}
                      {u.role === "Employee" ? "Unassigned" : u.role}
                    </option>
                  ))}
                </select>
              </div>
              <div className="um-field">
                ...
                <select
                  ...
                  value={selectedRole}
                  onChange={(e) =>
                    setSelectedRole(e.target.value as ManageRole)
                  }
                >
            ...
            <div className="um-modal-actions">
              ...
              <button
                ...
                disabled={isSaving || !selectedEmpId}
              >
