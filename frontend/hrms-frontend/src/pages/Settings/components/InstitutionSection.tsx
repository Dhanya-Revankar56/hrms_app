...
export default function InstitutionSection() {
  ...
  const handleSave = async () => {
    try {
      ...
      alert(
        "Failed to save valid settings. Ensure your payload limit is sufficient.",
      );
    }
  };
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setFormData((prev) => ({
          ...prev,
          institution_logo: reader.result as string,
        }));
      };
  ...
  const handleDayToggle = (day: string) => {
    ...
    if (current.includes(day)) {
      setFormData((prev) => ({
        ...prev,
        working_days: current.filter((d) => d !== day),
      }));
    } else {
  ...
  const renderField = (
    label: string,
    name: keyof InstitutionSettings,
    value: string,
  ) => {
      ...
      <div className="inst-grid">
          ...
          <div className="inst-card">
            ...
            <div className="info-grid" style={{ gridTemplateColumns: "1fr" }}>
              {renderField(
                "PAN",
                "pan_number",
                (isEditing ? formData.pan_number : settings.pan_number) || "",
              )}
              {renderField(
                "Registration No",
                "registration_number",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            ...
            <div className="info-grid">
              ...
              <div className="info-item">
                ...
                <span className="info-value">
                  {settings.institution_id || "—"}
                </span>
              </div>
              {renderField(
                "Owner Name",
                "owner_name",
                (isEditing ? formData.owner_name : settings.owner_name) || "",
              )}
              <div className="info-item">
                <span className="info-label">Added By</span>
                <span className="info-value">
                  {settings.added_by || "System Admin"}
                </span>
              </div>
              {renderField(
                "Short Name",
                ...
                (isEditing
                  ? formData.institution_short_name
              ...
              <div className="info-item">
                ...
                <span className="info-value">
                  {settings.created_at
                    ? new Date(parseInt(settings.created_at)).toLocaleString()
                    : "—"}
                </span>
              </div>
            </div>
          ...
          <div className="inst-card">
            ...
            <div className="info-grid">
              {renderField(
                "Organization email",
                "contact_email",
                (isEditing ? formData.contact_email : settings.contact_email) ||
                  "",
              )}
              {renderField(
                "Organization mobile",
              ...
              {renderField(
                "Organization phone",
                "contact_phone",
                (isEditing ? formData.contact_phone : settings.contact_phone) ||
                  "",
              )}
              ...
              {renderField(
                "Website",
                "website_url",
                (isEditing ? formData.website_url : settings.website_url) || "",
              )}
              {renderField(
                "Fax Number",
                "fax_number",
                (isEditing ? formData.fax_number : settings.fax_number) || "",
              )}
            </div>
          </div>
          ...
          <div className="inst-card">
            ...
            >
              Selected days will be treated as working days for leave
              calculations and holiday planners.
            </p>
          </div>
        </div>
      </div>
