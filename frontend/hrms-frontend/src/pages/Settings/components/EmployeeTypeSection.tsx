...
export default function EmployeeTypeSection() {
  const { data, refetch } = useQuery<{
    settings: { employee_types: Array<{ id: string; name: string }> };
  }>(GET_SETTINGS, {
    fetchPolicy: "network-only",
  });
  const [upsertEmployeeType] = useMutation(UPSERT_EMPLOYEE_TYPE, {
    onCompleted: () => refetch(),
  });
  ...
  return (
    <>
      <style>{CSS}</style>
      <SettingsSection
        title={`Employee Types (${types.length})`}
        description="Manage employment types (e.g. Regular, Contract, Intern)."
        icon={
          <svg
            width="18"
            height="18"
              ...
              <tr key={t.id}>
                ...
                <td>
                  <button
                    ...
                    onClick={() =>
                      deleteEmployeeType({ variables: { id: t.id } })
                    }
                  >
                    Delete
                  </button>
