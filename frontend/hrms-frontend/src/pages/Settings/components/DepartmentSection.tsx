...
export default function DepartmentsSection() {
  ...
  const [upsertDepartment, { loading: addLoading }] = useMutation(
    UPSERT_DEPARTMENT,
    {
      onCompleted: () => {
        refetch();
  ...
  async function handleAdd() {
    ...
    try {
      await upsertDepartment({
        variables: {
          input: { name: name.trim(), short_name: code.trim().toUpperCase() },
        },
      });
    } catch (e) {
            ...
            <table className="ds-table">
              ...
              <tbody>
                {departments.map((dept) => (
                  <tr key={dept.id}>
                    <td>
                      <span className="ds-dept-name">{dept.name}</span>
                    </td>
                    <td>
                      <span className="ds-code-pill">{dept.short_name}</span>
                    </td>
                    <td>
                      <button
