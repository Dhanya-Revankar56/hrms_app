...
export default function EmployeeDetail() {
  ...
  const { data: empData, loading: empLoading } = useQuery<any>(GET_EMPLOYEE_BY_ID, {
    variables: { id },
  ...
  const { data: salaryData } = useQuery<any>(GET_SALARY_RECORD, {
    variables: { employee_id: id },
    skip: activeTab !== "Salary Details" && activeTab !== "Summary",
  });

  const { data: attendanceData } = useQuery<any>(GET_ATTENDANCES, {
    variables: { employee_id: id },
    skip: activeTab !== "Attendance",
  });

  const { data: leavesData } = useQuery<any>(GET_LEAVES, {
    variables: { employee_id: id },
    skip: activeTab !== "Leaves",
  });

  useQuery<any>(GET_LEAVE_BALANCES, {
    variables: { employee_id: id },
    skip: activeTab !== "Leaves",
  });
  ...
  // Set default leave type when modal opens or leaveTypes load
  useEffect(() => {
    if (leaveTypes.length > 0 && !applyForm.leave_type) {
      setApplyForm(prev => ({ ...prev, leave_type: leaveTypes[0].name }));
    }
  }, [leaveTypes, applyForm.leave_type]);

  // Sync To Date with From Date for Half Days
  useEffect(() => {
    if (applyForm.is_half_day && applyForm.from_date) {
      setApplyForm(prev => ({ ...prev, to_date: prev.from_date }));
    }
  }, [applyForm.from_date, applyForm.is_half_day]);

  const { data: movementsData } = useQuery<any>(GET_MOVEMENTS, {
    variables: { employee_id: id },
    skip: activeTab !== "Movements",
  });

  const { data: docsData } = useQuery<any>(GET_EMPLOYEE_DOCUMENTS, {
    variables: { employee_id: id },
        ...
        {activeTab === "Summary" && (
          <div className="ed-content-grid">
            ...
            <div className="ed-section">
              ...
              {employee.bank_detail?.length > 0 ? (
                employee.bank_detail.map((bank: any, idx: number) => (
                  <div key={idx} style={{marginBottom: 16, borderBottom: idx < employee.bank_detail.length - 1 ? '1px solid #f1f5f9' : 'none', paddingBottom: 8}}>
        ...
        {activeTab === "Attendance" && (
          <div className="ed-section" style={{overflowX: 'auto'}}>
            ...
            <table className="ed-table">
              ...
              <tbody>
                {attendanceData?.attendances?.items?.map((att: any) => (
                  <tr key={att.id}>
          ...
          <div style={{width: '100%'}}>
            ...
            <div className="ed-section" style={{overflowX: 'auto'}}>
              ...
              <table className="ed-table">
                ...
                <tbody>
                  {movementsData?.movements?.items?.map((m: any) => (
                    <tr key={m.id}>
          ...
          <div style={{width: '100%'}}>
            ...
            <div className="ed-section" style={{overflowX: 'auto'}}>
              <table className="ed-table">
                ...
                <tbody>
                  {leavesData?.leaves?.items?.map((l: any) => (
                    <tr key={l.id}>
                    ...
                    <td>
                      <span className="ed-pill" style={{
                        background: (l.approvals?.find((a: any) => a.role === 'HOD')?.status === 'approved' ? '#dcfce7' : l.approvals?.find((a: any) => a.role === 'HOD')?.status === 'rejected' ? '#fee2e2' : '#fef9c3'),
                        color: (l.approvals?.find((a: any) => a.role === 'HOD')?.status === 'approved' ? '#166534' : l.approvals?.find((a: any) => a.role === 'HOD')?.status === 'rejected' ? '#991b1b' : '#854d0e')
                      }}>
                        {l.approvals?.find((a: any) => a.role === 'HOD')?.status || 'pending'}
                      </span>
                    </td>
                    <td>
                      <span className="ed-pill" style={{
                        background: (l.approvals?.find((a: any) => a.role === 'ADMIN')?.status === 'approved' ? '#dcfce7' : l.approvals?.find((a: any) => a.role === 'ADMIN')?.status === 'rejected' ? '#fee2e2' : '#fef9c3'),
                        color: (l.approvals?.find((a: any) => a.role === 'ADMIN')?.status === 'approved' ? '#166534' : l.approvals?.find((a: any) => a.role === 'ADMIN')?.status === 'rejected' ? '#991b1b' : '#854d0e')
                      }}>
                        {l.approvals?.find((a: any) => a.role === 'ADMIN')?.status || 'pending'}
                      </span>
                    </td>
                    <td>
                      <span className="ed-pill" style={{
              ...
              <div className="ed-modal-overlay">
                <div className="ed-modal">
                  ...
                  <div className="ed-modal-body">
                    ...
                    <div className="ed-filter-item">
                      <label className="ed-filter-label">Leave Type</label>
                      <select className="ed-select" value={applyForm.leave_type} onChange={e => setApplyForm({...applyForm, leave_type: e.target.value})}>
                        {leaveTypes.map((lt: any) => (
                  ...
                  <div className="ed-modal-foot">
                    ...
                    <button className="ed-btn ed-btn-primary" disabled={empLoading} onClick={async () => {
                      ...
                      const overlap = existingLeaves.some((l: any) => {
                      ...
                      try {
                        ...
                        setStatus({ message: err.message || "Error applying", type: 'error' });
                      }
        ...
        {activeTab === "Documents" && (
          <div className="ed-section">
            ...
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16, marginTop: 16}}>
              {docsData?.employeeDocuments?.map((doc: any) => (
          ...
          <div className="ed-modal" style={{ width: 500 }}>
            ...
            <div className="ed-modal-foot">
              ...
              <button className="ed-btn ed-btn-primary" disabled={mLoading} onClick={async () => {
                ...
                const overlap = existMv.some((m: any) => {
                ...
                try {
                  ...
                  setMStatus({ message: err.message || "Error applying", type: 'error' });
                }
