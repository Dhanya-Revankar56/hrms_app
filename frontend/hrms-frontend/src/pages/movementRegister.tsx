...
import { GET_MOVEMENTS, CREATE_MOVEMENT, UPDATE_MOVEMENT } from "../graphql/movementQueries";
...
const MV_REASONS: MvReason[] = [
  "Exam Duty","Bank Visit","Medical Appointment","Outside Meeting",
  "Personal Emergency","Official Field Work","Government Office","Other",
];
...
function SortIco({ on, dir }: { on: boolean; dir: SortDir }) {
  return (
    <svg width="9" height="9" fill="none" viewBox="0 0 24 24"
      stroke={on ? "#4f46e5" : "#d1d5db"} strokeWidth={2.5}>
      {on && dir === "asc"
...
interface SThP { field: SortField; label: string; sf: SortField; sd: SortDir; onSort:(f:SortField)=>void; }
function STh({ field, label, sf, sd, onSort }: SThP) {
  return (
    <th className={sf === field ? "mr-sorted" : ""} onClick={() => onSort(field)}>
      <div className="mr-th-i">{label}<SortIco on={sf === field} dir={sd} /></div>
    </th>
  );
}

interface DFP { label: string; value: string; num?: boolean; }
...
function NewModal({ onClose, onToast }: ModalP) {
  ...
  const blank: FormState = { empId:"", date:TODAY, outTime:"", returnTime:"", reason:"Exam Duty", reasonDetail:"" };
  ...
  return (
    <div className="mr-mo" onClick={onClose}>
      <div className="mr-mi" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
        ...
        <div className="mr-mi-b">
          <div className="mr-fg2">
            <div>
              <label className="mr-lbl">Employee <span className="mr-req">*</span></label>
            ...
            <div>
              <label className="mr-lbl">Reason <span className="mr-req">*</span></label>
              <select className="mr-sel2" value={form.reason}
                onChange={(e) => f("reason", e.target.value as MvReason)}>
                {MV_REASONS.map((r: MvReason) => <option key={r} value={r}>{r}</option>)}
              </select>
            </div>
          </div>
          <div className="mr-fg4">
            <div>
              <label className="mr-lbl">Date <span className="mr-req">*</span></label>
              <input type="date" className={`mr-inp${errs.date?" mr-ferr":""}`} value={form.date}
                onChange={(e) => f("date", e.target.value)}/>
              {errs.date && <div className="mr-errt">{errs.date}</div>}
            </div>
            <div>
              <label className="mr-lbl">Out Time <span className="mr-req">*</span></label>
              <div className={`mr-time-trigger${errs.outTime?" err":""}`} onClick={() => setShowOut(true)}>
              ...
              {showOut && (
                <AnalogTimePicker 
                  ...
                  onSave={(t) => { f("outTime", t); setShowOut(false); }}
                  onCancel={() => setShowOut(false)}
                />
              )}
            </div>
            <div>
              <label className="mr-lbl">Return By <span className="mr-req">*</span></label>
              <div className={`mr-time-trigger${errs.returnTime?" err":""}`} onClick={() => setShowRet(true)}>
                <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
                {to12(form.returnTime)}
              </div>
              {errs.returnTime && <div className="mr-errt">{errs.returnTime}</div>}
              {showRet && (
                <AnalogTimePicker 
                   initialTime={form.returnTime || "10:00"}
                   onSave={(t) => { f("returnTime", t); setShowRet(false); }}
            ...
            <div>
              ...
              <div className="mr-fg2">
                <div>
                  ...
                  <input className="mr-inp" value={selectedEmp?.work_detail?.department?.name || "N/A"} readOnly style={{ background:"#f9fafb", color:"#6b7280" }}/>
                </div>
                <div>
                  <label className="mr-lbl">Designation</label>
                  <input className="mr-inp" value={selectedEmp?.work_detail?.designation?.name || "N/A"} readOnly style={{ background:"#f9fafb", color:"#6b7280" }}/>
                </div>
              </div>
          <div className="mr-ff">
            <label className="mr-lbl">Details <span className="mr-req">*</span></label>
            <textarea className={`mr-ta${errs.reasonDetail?" mr-ferr":""}`}
              placeholder="Describe the purpose of this movement…"
              value={form.reasonDetail}
              onChange={(e) => f("reasonDetail", e.target.value)}/>
            {errs.reasonDetail && <div className="mr-errt">{errs.reasonDetail}</div>}
          </div>
        </div>
        <div className="mr-mi-f">
          <button className="mr-mi-btn mr-mi-out" onClick={onClose}>Cancel</button>
          <button className="mr-mi-btn mr-mi-pri" onClick={submit}>Submit Request</button>
        </div>
      </div>
    </div>
  );
...
function Drawer({ rec, onClose, onToast, initialIsEditing, onUpdate }: DrP & { onUpdate: (vars: { variables: { id: string, input: { status?: string, admin_status?: string, dept_admin_status?: string, remarks?: string, admin_remarks?: string, dept_admin_remarks?: string, in_time?: string, out_time?: string } } })=>void }) {
  ...
  function logRet() {
    if (!ret) { onToast("Enter a return time."); return; }
    ...
    if (moveDateStr === todayStr) {
      ...
      if (curTime < rec.out_time) {
        onToast(`You cannot log a return before the out-time (${to12(rec.out_time)}).`);
        return;
      }
    }

    if (ret <= rec.out_time) { onToast("Must be after out time."); return; }
    ...
    <>
      ...
      <div className="mr-dr">
        <div className="mr-dr-hd">
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div className="mr-dr-av" style={{ background: "#1d4ed8" }}>
              {initials(rec.employee?.first_name || "?", rec.employee?.last_name || "?")}
            </div>
            <div style={{ flex: 1 }}>
              <div className="mr-dr-nm">{rec.employee?.first_name} {rec.employee?.last_name || "Unknown"}</div>
              <div className="mr-dr-mt">{rec.employee_code} · {rec.employee?.work_detail?.department?.name || "N/A"}</div>
              <div style={{ marginTop:6 }}><StatusCell status={rec.status}/></div>
            </div>
          </div>
        ...
        <div className="mr-dr-bd">
            ...
            <div className="mr-edit-box">
              ...
              <div className="mr-edit-f">
                <div>
                  ...
                  <div className="mr-time-trigger" onClick={() => setShowOutP(true)}>
                    {to12(editOut)}
                  </div>
                  {showOutP && (
                    <AnalogTimePicker 
                      initialTime={editOut} 
                      onSave={(t) => { setEditOut(t); setShowOutP(false); }}
                ...
                <div>
                  ...
                  <div className="mr-time-trigger" onClick={() => setShowRetP(true)}>
                    {to12(editRet)}
                  </div>
                  {showRetP && (
                    <AnalogTimePicker 
                      initialTime={editRet} 
                      onSave={(t) => { setEditRet(t); setShowRetP(false); }}
          ...
          <div className="mr-sec">
            ...
            <div className="mr-tv">
              <div className="mr-tv-b">
                ...
                <div className="mr-tv-t" style={{ color:"#d97706" }}>{to12(rec.out_time)}</div>
                <div className="mr-tv-n">{fmtDate(rec.movement_date)}</div>
              </div>
              <div className="mr-tv-b">
                <div className="mr-tv-lbl">Expected Return</div>
                <div className="mr-tv-t" style={{ color:"#d97706" }}>{to12(rec.in_time)}</div>
                <div className="mr-tv-n">Expected</div>
              </div>
              <div className="mr-tv-b">
                <div className="mr-tv-lbl">Actual Return</div>
                <div className="mr-tv-t" style={{ color: actualReturn ? (late ? "#dc2626" : "#16a34a") : "#9ca3af" }}>
                  {actualReturn ? to12(actualReturn) : "—"}
                </div>
                <div className="mr-tv-n">{actualReturn ? (late ? "⚠ Late" : "On time") : "Pending"}</div>
              </div>
            </div>
          </div>
          {/* Details */}
          <div className="mr-sec">
            ...
            <div className="mr-grid2">
              <DF label="Employee ID"  value={rec.employee?.employee_id || rec.employee_code || ""} num />
              <DF label="Designation"  value={rec.employee?.work_detail?.designation?.name || "N/A"} />
              <DF label="Date"         value={fmtDate(rec.movement_date)} />
              <DF label="Reason"       value={rec.movement_type} />
              <DF label="Plan (Out - In)" value={`${to12(rec.out_time)} - ${to12(rec.in_time)}`} />
            </div>
          </div>
          {/* Description */}
          <div className="mr-sec">
          ...
          <div className="mr-sec">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ padding: "14px 16px", borderRadius: "10px", background: "#f8fafc", border: "1px solid #e8edf5" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#334155" }}>Department Admin Status</span>
                  <StatusCell status={rec.dept_admin_status || "Pending"} />
                </div>
                {rec.dept_admin_remarks && <div style={{ fontSize: 13, color: "#64748b", fontStyle: "italic", marginTop: 8 }}>"{rec.dept_admin_remarks}"</div>}
              </div>
              <div style={{ padding: "14px 16px", borderRadius: "10px", background: "#f8fafc", border: "1px solid #e8edf5" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#334155" }}>Admin Status</span>
                  <StatusCell status={rec.admin_status || "Pending"} />
                </div>
                {rec.admin_remarks && <div style={{ fontSize: 13, color: "#64748b", fontStyle: "italic", marginTop: 8 }}>"{rec.admin_remarks}"</div>}
              </div>
            </div>
          </div>
          ...
          {(!rec.dept_admin_status || rec.dept_admin_status?.toLowerCase() === "pending") && (
            <div className="mr-sec">
              ...
              <textarea className="mr-rem" placeholder="Dept Admin remarks…"
                value={deptRem} onChange={(e) => setDeptRem(e.target.value)}/>
              <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
                <button className="mr-dr-btn mr-dr-reject" onClick={() => handleAction("HOD", "Rejected", deptRem)}>Reject as Dept Admin</button>
                <button className="mr-dr-btn mr-dr-approve" onClick={() => handleAction("HOD", "Approved", deptRem)}>Approve as Dept Admin</button>
              </div>
            </div>
          )}
          ...
          {(!rec.admin_status || rec.admin_status?.toLowerCase() === "pending") && (
            <div className="mr-sec">
              ...
              <textarea className="mr-rem" placeholder="Admin remarks…"
                value={adminRem} onChange={(e) => setAdminRem(e.target.value)}/>
              <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
                <button className="mr-dr-btn mr-dr-reject" onClick={() => handleAction("ADMIN", "Rejected", adminRem)}>Reject as Admin</button>
                <button className="mr-dr-btn mr-dr-approve" onClick={() => handleAction("ADMIN", "Approved", adminRem)}>Approve as Admin</button>
              </div>
            </div>
          )}
            ...
            <div className="mr-sec">
              ...
              <div className="mr-rl">
                ...
                <div className="mr-rl-row">
                  <input type="time" className="mr-rl-in" value={ret}
        ...
        <div className="mr-dr-ft">
          <button className="mr-dr-btn mr-dr-close" style={{ flex:"unset", width:"100%" }} onClick={onClose}>Close</button>
        </div>
      </div>
    </>
  );
  ...
  const filtered = useMemo<MovementRecord[]>(() => {
    ...
    if (flt.search) {
      ...
      list = list.filter((r: MovementRecord) => {
        ...
        return fullName.includes(q) || r.employee_code.toLowerCase().includes(q);
      });
    }
    if (flt.month)            list = list.filter((r: MovementRecord) => r.movement_date.startsWith(flt.month));
    ...
    list.sort((a: MovementRecord, b: MovementRecord) => {
      ...
      if (sf === "date")   { va = a.movement_date + a.out_time; vb = b.movement_date + b.out_time; }
      if (sf === "status") { va = a.status; vb = b.status; }
    ...
    <>
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:24 }}>
        <div>
          ...
          <div className="mr-sub" style={{ marginBottom:0 }}>Welcome to your approvals board! Here you can approve the movement requests by Employees.</div>
        </div>
        <button className="mr-add" onClick={() => setModal(true)}>
      ...
      <div className="mr-filter-bar">
        ...
        <div className="mr-search-wrap">
          <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.2} className="mr-search-icon">
            <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-4.35-4.35M17 11A6 6 0 115 11a6 6 0 0112 0z"/>
          </svg>
          <input className="mr-search" placeholder="Search Employee..."
            value={flt.search}
        ...
        {/* Department */}
        <select className="mr-filter-sel" value={flt.department}
        ...
        {/* Status */}
        <select className="mr-filter-sel" value={flt.status}
          onChange={(e) => setF("status", e.target.value as FilterState["status"])}
          style={{ width: "135px" }}>
        ...
        {/* Refresh */}
        <button className="mr-refresh" onClick={() => {
          setSpin(true); setTimeout(() => setSpin(false), 500);
          showToast("Records refreshed.");
        }} style={{ height: "36px", padding: "0 12px" }}>
          <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
            style={{ transition:"transform .5s", transform: spin ? "rotate(360deg)" : "none" }}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
          </svg>
          Refresh
        </button>

        <div className="mr-filter-count">{filtered.length} Requests</div>
      </div>
      {/* ── Table card ── */}
        ...
        <div className="mr-table-wrap">
          {filtered.length === 0 ? (
            <div className="mr-empty">
              ...
              <span style={{ fontSize:13, color:"#9ca3af" }}>Try adjusting your search or category selection.</span>
            </div>
          ) : (
            <table className="mr-table">
              <thead>
                <tr>
                  <STh field="empId"  label="Employee ID" sf={sf} sd={sd} onSort={sort}/>
                  <STh field="empName" label="Employee Name" sf={sf} sd={sd} onSort={sort}/>
                  <STh field="date"   label="Movement Date" sf={sf} sd={sd} onSort={sort}/>
                  <th>Movement Time</th>
                  <th>Reason</th>
                  <th>Status</th>
              ...
              <tbody>
                {filtered.map((rec: MovementRecord) => (
                  <tr key={rec.id} onClick={() => setDrawer(rec)}>
                    <td><span className="mr-empid">{rec.employee?.employee_id || rec.employee_code || "—"}</span></td>
                    <td><span className="mr-name">{rec.employee?.first_name} {rec.employee?.last_name}</span></td>
                    <td><span className="mr-date">{fmtDate(rec.movement_date)}</span></td>
        ...
        {pageInfo && pageInfo.totalPages > 1 && (
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 20px", borderTop:"1px solid #f3f4f6" }}>
            <div style={{ fontSize:"13px", color:"#6b7280" }}>
              Showing Page <strong>{pageInfo.currentPage}</strong> of <strong>{pageInfo.totalPages}</strong> ({pageInfo.totalCount} total records)
            </div>
            <div style={{ display:"flex", gap:"8px" }}>
              <button 
                className="mr-refresh" 
                style={{ height:"32px", opacity: currentPage === 1 ? 0.5 : 1 }}
