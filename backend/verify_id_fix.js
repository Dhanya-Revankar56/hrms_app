...
async function test() {
  ...
  console.log(`Testing with Employee: ${emp.name}, _id: ${emp._id}, user_id: ${emp.user_id}`);
  ...
  try {
    ...
    // Cleanup
    await mongoose.connection.db.collection('leaves').deleteOne({ _id: result._id });
  ...
  try {
    ...
    console.log('✅ Movement Applied! Resolved Employee ID:', result.employee_id);

    // Cleanup
    await mongoose.connection.db.collection('movements').deleteOne({ _id: result._id });
