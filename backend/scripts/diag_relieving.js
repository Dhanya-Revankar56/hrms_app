...
async function diagnostic() {
  ...
  if (!user) {
    ...
    const someUsers = await mongoose.connection.db.collection('users').find({}).limit(5).toArray();
...
async function runDiagnostic(user) {
  console.log('User Info:', { id: user._id, email: user.email, role: user.role, tenant_id: user.tenant_id });

  // 1. Find Employee record
  const emp = await mongoose.connection.db.collection('employees').findOne({ user_id: user._id });
  console.log('Employee Record:', emp ? { id: emp._id, employee_id: emp.employee_id, tenant_id: emp.tenant_id, dept: emp.work_detail?.department } : 'NOT FOUND');

  if (emp) {
    // 2. Find Relieving records for this employee
    const relievings = await mongoose.connection.db.collection('relievings').find({ 
      $or: [
        { employee_id: emp._id },
    ...
    console.log(`Relieving Records found for employee (${emp._id}):`, relievings.length);
    ...
    const allRelievings = await mongoose.connection.db.collection('relievings').find({ 
        tenant_id: emp.tenant_id 
    }).toArray();
    console.log(`Total Relievings for tenant (${emp.tenant_id}):`, allRelievings.length);
