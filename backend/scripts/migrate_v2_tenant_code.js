...
async function migrate() {
  try {
    ...
    for (const user of users) {
      ...
      const tenant = await Tenant.findById(user.tenant_id).setOptions({
        skipTenant: true,
      });
      if (!tenant) {
        console.log(
          `❌ Tenant ID ${user.tenant_id} not found for user ${user.email}. Skipping.`,
        );
        continue;
      }
      ...
      console.log(
        `✅ Updated User ${user.email} with tenant_code: ${tenant.code}`,
      );
    }
