...
async function migrate() {
  ...
  try {
    ...
    const institutionCodes = await Employee.distinct(
      "institution_id",
    ).setOptions({ skipTenant: true });
    console.log(
      `🔍 Found ${institutionCodes.length} institutions: ${institutionCodes.join(", ")}`,
    );
    ...
    for (const code of institutionCodes) {
      ...
      let tenant = await Tenant.findOne({ code: upperCode }).setOptions({
        skipTenant: true,
      });
      if (!tenant) {
    ...
    for (const emp of employees) {
      ...
      if (!tId) {
        results.errors.push(
          `Employee ${emp.employee_id || emp._id} has NO valid institution_id`,
        );
        continue;
      }
      ...
      if (existingUser) {
        ...
        try {
          const newUser = new User({
            email: (
              emp.user_email || `user_${emp.employee_id || emp._id}@temp.com`
            ).toLowerCase(),
          ...
          results.errors.push(
            `USER_CREATE_FAILED: ${emp.user_email} -> ${uErr.message}`,
          );
          continue;
        }
      }
