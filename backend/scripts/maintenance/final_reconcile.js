    ...
    for (const emp of employees) {
        ...
        if (currentDept && !mongoose.Types.ObjectId.isValid(currentDept)) {
            ...
            let dept = await Department.findOneAndUpdate(
                { institution_id: INST_ID, name: { $regex: new RegExp("^" + name + "$", "i") } },
        ...
        if (currentDesg && !mongoose.Types.ObjectId.isValid(currentDesg)) {
            ...
            let desg = await Designation.findOneAndUpdate(
                { institution_id: INST_ID, name: { $regex: new RegExp("^" + name + "$", "i") } },
                { $setOnInsert: { institution_id: INST_ID, name, department: mongoose.Types.ObjectId.isValid(deptToUse) ? deptToUse : null, is_active: true } },
                { upsert: true, new: true }
            );
        ...
        if (changed) {
            await Employee.updateOne(
                { _id: emp._id },
                { $set: { "work_detail.department": updatedDept, "work_detail.designation": updatedDesg } }
            );
