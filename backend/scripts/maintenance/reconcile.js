...
async function run() {
  try {
    ...
    console.log(`📋 Processing ${employees.length} employees for ${INST_ID}...`);

    for (const emp of employees) {
        console.log(`🔍 Checking employee: ${emp.firstName || emp.first_name} | Dept: ${emp.work_detail?.department} | Desg: ${emp.work_detail?.designation}`);
        let changed = false;
        const newWorkDetail = { ...emp.work_detail };
        
        // Handle Department
        if (newWorkDetail.department && !mongoose.Types.ObjectId.isValid(newWorkDetail.department)) {
            const deptName = String(newWorkDetail.department).trim();
            let dept = await Department.findOne({ institution_id: INST_ID, name: deptName });
            if (!dept) {
        ...
        // Handle Designation
        if (newWorkDetail.designation && !mongoose.Types.ObjectId.isValid(newWorkDetail.designation)) {
            const desgName = String(newWorkDetail.designation).trim();
            const deptId = newWorkDetail.department; 
            
            let desg = await Designation.findOne({ institution_id: INST_ID, name: desgName });
            if (!desg) {
        ...
        if (changed) {
            const upResult = await Employee.updateOne({ _id: emp._id }, { $set: { work_detail: newWorkDetail } });
            console.log(`   ✅ Migrated employee: ${emp.first_name} ${emp.last_name} | Modified: ${upResult.modifiedCount}`);
        } else {
