...
async function sync() {
  ...
  try {
    ...
    if (otherTenantIds.length > 0) {
      console.log(
        `⚠️ Found ${otherTenantIds.length} duplicate(s). Consolidating...`,
      );
    }
    ...
    if (otherTenantIds.length > 0) {
      const delRes = await db
        .collection("tenants")
        .deleteMany({ _id: { $in: otherTenantIds } });
      console.log(`🗑 Duplicate Tenants Deleted: ${delRes.deletedCount}`);
    }

    // 5. Final Audit for Dhanya
    const dhanya = await db
      .collection("users")
      .findOne({ email: "dhanyavernekar@gmail.com" });
    const employeesInTenantCount = await db
      .collection("employees")
      .countDocuments({ tenant_id: masterTenant._id });
