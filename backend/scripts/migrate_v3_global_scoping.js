...
async function migrate() {
  try {
    ...
    for (const modelInfo of MODELS) {
      ...
      const query = (modelInfo.name === "MovementRegister" || modelInfo.name === "Employee") 
        ? {} 
        : { 
            $or: [
              { tenant_id: { $exists: false } },
              { tenant_id: null },
              ...(modelInfo.name === "User" ? [{ tenant_code: { $exists: false } }] : [])
            ]
          };

      const records = await Model.find(query).setOptions({ skipTenant: true }).lean();
      ...
      for (const rec of records) {
        ...
        if (rec.employee_id && typeof rec.employee_id === "string") {
          try { update.employee_id = new mongoose.Types.ObjectId(rec.employee_id); } catch(e) {}
        }
        if (rec.user_id && typeof rec.user_id === "string") {
          try { update.user_id = new mongoose.Types.ObjectId(rec.user_id); } catch(e) {}
        }

        if (Object.keys(update).length > 0) {
          await Model.updateOne({ _id: rec._id }, { $set: update }, { skipTenant: true });
        }
      }
