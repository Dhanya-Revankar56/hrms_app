...
async function test() {
  ...
  storage.run(tenantId, async () => {
    // 1. Test Holidays
    const Holiday = mongoose.model('Holiday', new mongoose.Schema({ tenant_id: mongoose.Schema.Types.ObjectId, date: Date, is_active: Boolean }));
    ...
    const Employee = mongoose.model('Employee', new mongoose.Schema({ user_id: mongoose.Schema.Types.ObjectId, tenant_id: mongoose.Schema.Types.ObjectId }));
    const emp = await Employee.findOne(withTenant({ user_id: user._id })).lean();
    console.log('Employee found for user:', emp ? emp._id : 'NONE');

    const Relieving = mongoose.model('Relieving', new mongoose.Schema({ employee_id: mongoose.Schema.Types.ObjectId, tenant_id: mongoose.Schema.Types.ObjectId }));
    if (emp) {
