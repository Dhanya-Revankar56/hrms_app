...
async function test() {
  ...
  await runWithTenant({ tenantId: tenant._id }, async () => {
    ...
    if (!emp) {
      console.log("Emp not found for user");
      return;
    }
    const deptId = emp.work_detail.department.toString();

    console.log(
      `Testing with HOD: ${user.email}, Role: ${user.role}, Dept: ${deptId}`,
    );

    const ctx = {
      user: { id: user._id.toString(), role: user.role, tenant_id: tenant._id },
    };
    ...
    const allSameDept = emps.items.every(
      (item) => item.work_detail.department.toString() === deptId,
    );
    if (allSameDept) {
