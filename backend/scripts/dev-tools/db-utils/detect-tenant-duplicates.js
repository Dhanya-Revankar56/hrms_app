...
async function detectDuplicates() {
  try {
    ...
    if (emailDuplicates.length === 0) {
      ...
      console.log(
        `⚠️ Found ${emailDuplicates.length} emails used multiple times:`,
      );
      emailDuplicates.forEach((dup) => {
        ...
        console.log(
          `    Type: ${isCrossTenant ? "CROSS-TENANT (Will be allowed)" : "SAME-TENANT (MUST BE FIXED manually)"}`,
        );
        console.log("    ---");
      });
    }
    ...
    if (idDuplicates.length === 0) {
      ...
      console.log(
        `⚠️ Found ${idDuplicates.length} Employee IDs used multiple times:`,
      );
