...
async function testFetch() {
  ...
  if (!user) {
    console.log(
      "User dhanyavernekar@gmail.com not found. Cannot verify with correct tenant context.",
    );
  ...
  await runWithTenant(tenantId, async () => {
    // 1. Test Relievings
    console.log("\n--- Testing Relievings ---");
    const relResult = await relievingService.listRelievings({
    ...
    if (relResult.items.length > 0) {
      ...
      console.log(
        "Example Relieving Employee:",
        relResult.items[0].employee_id?.first_name || "NOT POPULATED",
      );
    }
