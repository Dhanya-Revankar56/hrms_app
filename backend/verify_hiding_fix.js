...
async function test() {
  ...
  await runWithTenant({ tenantId: tenant._id }, async () => {
    ...
    const emp = await Employee.findOne({ name: 'Relieved Test', tenant_id: tenant._id }).lean();
    if (!emp) {
        console.log('Relieved Test record not found. Please create one by relieving an employee first.');
        return;
    }
    ...
    const foundInManagement = employeeList.items.find(e => e._id.toString() === emp._id.toString());
    
    if (!foundInManagement) {
        console.log('✅ Relieved employee HIDDEN from Employee Management list');
    } else {
        console.log('❌ Relieved employee still VISIBLE in Employee Management list');
    }

    // TEST 2: listRelievings (Relieving Module)
    const relievingList = await listRelievings({ pagination: { limit: 100 } });
    const foundInRelieving = relievingList.items.find(r => r.employee_id?._id?.toString() === emp._id.toString());
    
    if (foundInRelieving) {
        console.log('✅ Relieved employee data successfully POPULATED in Relieving list');
        console.log(`   (Department: ${foundInRelieving.employee_id?.work_detail?.department || 'unknown'})`);
    } else {
