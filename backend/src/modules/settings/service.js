...
exports.upsertMasterData = async (type, input, context) => {
  ...
  await AuditLog.create({
    ...
    metadata: {
      ...
      id: updated._id,
      name: updated.name,
      description: `${updated.name} ${actionBase.toLowerCase().replace(/_/g, " ")} is ${id ? "updated" : "created"}`,
    },
  });
...
exports.deleteMasterData = async (type, id, context) => {
  ...
  await AuditLog.create({
    ...
    metadata: {
      ...
      id,
      name: deleted.name,
      description: `${deleted.name} ${actionBase.toLowerCase().replace(/_/g, " ")} has been deleted`,
    },
  });
