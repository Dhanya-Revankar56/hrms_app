...
const checkMovementRulesMock = (ms, movement_date, out_time, in_time, existingCount) => {
...
try {
  const today = new Date();
  checkMovementRulesMock(mockSettings.movement_settings, today, "10:00", "10:30", 0);
...
try {
  ...
  checkMovementRulesMock(mockSettings.movement_settings, future, "10:00", "12:00", 0);
...
try {
  const future = new Date(); future.setDate(future.getDate() + 5);
  checkMovementRulesMock(mockSettings.movement_settings, future, "10:00", "10:30", 2);
...
assert.strictEqual(calculateStatusMock("approved", "pending"), "approved", "HOD approval should result in approved status");
console.log("✅ Passed: HOD Approval logic");

// Test 5: Admin Rejection Override
assert.strictEqual(calculateStatusMock("approved", "rejected"), "rejected", "Admin rejection should override HOD approval");
